...
const utilsGDrive = new UtilsGDrive(null, {
  expBack: {
    shouldRetry: err => { console.error(err); return false }
  }
});
...
describe.concurrent('methods', () => {
  ...
  test.concurrent('updateFiles()', async () => {
    ...
    const fileId = await utilsGDrive.upload({ localPath, parentIdentifiers: parentId });
    const res = await utilsGDrive.updateFiles({ fileId, resource: { name: fileName2 } });
  ...
  describe.concurrent('getFileId()', () => {
    ...
    test('file id given as object property', async () => {
      const fileId = await utilsGDrive.getFileId({ fileName: childName, parentId });

      expect(fileId).toEqual(childId);
    }, timeout)

    test('filename given as string', async () => {
      const fileId = await utilsGDrive.getFileId(childName);

      expect(fileId).toEqual(childId);
    }, timeout)

    test('path given as string', async () => {
  ...
  test('makeFolder()', async () => {
    const folderName = generateFolderName('makeFolder');

    const folderId = await utilsGDrive.makeFolder({ folderName, parentIdentifiers: parentId });
  ...
  describe.concurrent('move()', () => {
    ...
    test('move file', async () => {
      ...
      const fileIdGDrive = await utils.resolveId(utilsGDrive, { fileName, parentId: childId });
  ...
  describe.concurrent('rename()', () => {
    ...
    test('rename folder', async () => {
      ...
      const fileId = await utilsGDrive.makeFolder({ folderName: folderName1, parentIdentifiers: parentId });
