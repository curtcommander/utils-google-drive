...
beforeAll(() => {
    if (!fs.existsSync(pathCredentials)) {
        ...
        clientIdTest = parseClientId(JSON.parse(fs.readFileSync(pathCredentials).toString()));
    }
    ...
    if (!fs.existsSync(pathToken)) {
        ...
        accessTokenTest = parseAccessToken(JSON.parse(fs.readFileSync(pathToken).toString()));
    }
})
...
describe.concurrent('initialization', () => {
    ...
    test('initialize with token as string', () => {
        ...
        const accessTokenInit = utilsGDrive.drive.context._options.auth.credentials.access_token;
