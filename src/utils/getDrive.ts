...
function resolveToken(auth: Auth$UtilsGDrive = {}): Token$GoogleApi {
  if (auth.token) { 
    if (typeof auth.token === 'string') return destructToken(JSON.parse(auth.token));
    ...
    try {
      ...
      throw new UtilsGDriveError('Token not found. Consider running the script getTokenGDrive.js included in this package.');
    }
  }
}
